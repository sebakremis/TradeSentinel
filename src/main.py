...
EMA_FAST_PERIOD = 20
EMA_SLOW_PERIOD = 50
DISPLAY_COLUMNS = ['Ticker', 'Sector', 'Start Price', 'Close', 'Change %', 'Avg Return', 'Annualized Vol', 'Sharpe Ratio', 'Trend', 'Highest Close', 'Lowest Close', 'Distance HC' ]
...
def _format_final_df(final_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # This step might create 'Sector' as a column of np.nan (float type)
    for col in DISPLAY_COLUMNS:
        if col not in df.columns:
            df[col] = np.nan
    
    # 2. 🚨 CRITICAL FIX: Explicitly ensure 'Sector' column is a string type.
    # This must happen AFTER all steps that might create it or modify its type.
    if 'Sector' in df.columns:
        # Fill any NaN (which are floats) with 'N/A' and explicitly convert to string type
        # The .fillna('N/A') is crucial to handle NaNs before astype(str).
...
def _load_and_process_data(PeriodOrStart= "1y") -> (pd.DataFrame, pd.DataFrame, list): 
    ...
    # 🚨 NEW STEP 2A: Calculate Start Price for each Ticker
    # Group by Ticker and get the first Close price for the period
    start_prices = df_daily.groupby('Ticker')['Close'].first().reset_index()
    start_prices.rename(columns={'Close': 'Start Price'}, inplace=True)

    # Merge the new Start Price column onto the snapshot DataFrame
